...
# <img src="app/static/logo.png" alt="unmonitarr logo" width="30"> unmonitarr

**Stop Overseerr/Jellyseerr from auto-monitoring everything it requests.**

[![Build and publish](https://github.com/nicolas-gatta/unmonitarr/actions/workflows/docker-publish.yml/badge.svg)](https://github.com/nicolas-gatta/unmonitarr/actions/workflows/docker-publish.yml)
[![License: GPL v3](https://img.shields.io/badge/License-GPLv3-blue.svg)](LICENSE)
[![Python](https://img.shields.io/badge/python-3.12-4fb0a5.svg)](https://www.python.org/)
[![Docker](https://img.shields.io/badge/container-GHCR-4fb0a5.svg)](https://github.com/nicolas-gatta/unmonitarr/pkgs/container/unmonitarr)
...
You know it, I know it and everyone know it, [Overseerr](https://overseerr.dev/) and [Jellyseerr](https://github.com/Fallenbagel/jellyseerr) are great front ends for requesting media but when a request gets approved, it automatically add the movie/series to [Radarr](https://radarr.video/) or [Sonarr](https://sonarr.tv/) as **monitored** with no setting to change that. There's no built-in way to say "add it, but don't auto-monitor it,".

**unmonitarr** sits between them and Radarr/Sonarr: it listens for the request-approved webhook and flips the item to unmonitored the moment it's added - once, and only once. Anything you monitor manually afterwards is never touched again.

## Features

- 🪝 **Easy Webhook** use in Overseerr/Jellyseerr
...
services:
  unmonitarr:
    image: ghcr.io/nicolas-gatta/unmonitarr:latest
    container_name: unmonitarr
    restart: unless-stopped
    ports:
      - "5056:5055"
    volumes:
      - /opt/unmonitarr/data:/app/data
    environment:
      - TZ=Europe/Brussels
      - LOG_LEVEL=INFO
```
...
```bash
mkdir -p /opt/unmonitarr/data
chown -R 1000:1000 /opt/unmonitarr/data   # matches the container's fixed UID
docker compose up -d
```
...
5. *(Optional)* if you set a webhook secret on the unmonitarr settings page, add a matching `Authorization` header here
...
|---|---|---|
| Radarr URL / API key | Where unmonitarr reaches your Radarr instance | - |
| Sonarr URL / API key | Where unmonitarr reaches your Sonarr instance | - |
...
```
Overseerr/Jellyseerr ──(webhook: request approved)──▶ unmonitarr ──▶ Radarr/Sonarr API
                                                            │
                                                     set monitored=false
```
