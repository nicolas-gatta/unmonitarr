...
log = logging.getLogger("unmonitarr.webhook")
...
def webhook():
    ...
    if media_type == "movie" and tmdb_id:
        radarr = RadarrClient(cfg)
        threading.Thread(
            target=radarr.unmonitor,
            args=(tmdb_id, stats_module.record_unmonitor),
            daemon=True,
        ).start()
    elif media_type == "tv" and tvdb_id:
        sonarr = SonarrClient(cfg)
        threading.Thread(
            target=sonarr.unmonitor,
            args=(tvdb_id, stats_module.record_unmonitor),
            daemon=True,
        ).start()
    else:
